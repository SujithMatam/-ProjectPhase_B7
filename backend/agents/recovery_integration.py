...
# ============================================================================
# CONVERSATIONAL PATIENT-FACING TEMPLATES
#
# These functions still produce fully deterministic, source-gated wording --
# nothing here is generated by an LLM, and every clinical fact they state
# (checkpoint day vs. the patient's own day, the stated range, which
# reason a verdict could not be produced) comes straight from the
# CheckpointResult / DecisionResult passed in. What changed from the
# earlier pass is PHRASING ONLY: replacing report-style constructions
# ("has been noted", "your reported X", "there isn't a stated target to
# compare against yet") with the way a person would actually say the same
# grounded fact out loud, and rotating a small pool of acknowledgment
# phrases (via `_pick`, seeded from data already in the CheckpointResult/
# ask-count -- never randomness) so consecutive turns don't read identically.
# No wording here ever invents a target, a range, or a trajectory claim
# that isn't already present in the underlying data.
# ============================================================================
...
_ASK_QUESTIONS: Dict[str, str] = {
    recovery_logic.ROM_FLEXION_DEGREES: "About how many degrees can you currently bend your knee?",
    recovery_logic.ROM_EXTENSION_DEGREES: "Do you know your current knee extension measurement in degrees (how close to fully straight)?",
    recovery_logic.MOBILITY_STATUS: "How are you currently getting around -- walking independently, or using a walker, cane, or crutches?",
    recovery_logic.PAIN_SCORE: "On a scale of 0 to 10, what is your current pain level?",
}

# Acknowledgment shown ONLY when there is something to acknowledge -- a
# retried question (patient said they didn't know), a clarification
# (ambiguous answer), or returning to a question still outstanding. A
# genuinely first-time ask gets no prefix; there is nothing to acknowledge
# yet.
_RETRY_ACK_POOL: Tuple[str, ...] = ("No worries.", "That's okay.", "All good.")
_AMBIGUOUS_ACK_POOL: Tuple[str, ...] = (
    "Just want to make sure I've got that right.",
    "I want to double check that before I note it down.",
)
_PENDING_ACK_POOL: Tuple[str, ...] = (
    "Just circling back to this --",
    "Coming back to my last question --",
)
...
def format_ask_question(
    ...
    *,
    ...
    """
    Patient-facing question for one Recovery field. `reason_code` (a
    recovery_logic.DecisionReasonCode value) says WHY this question is being
    asked right now -- a plain first ask gets no acknowledgment, while a
    retry / clarification / return-to-a-pending-question gets a short,
    natural lead-in. `variation_seed` (the caller's own ask_count_of(metric)
    is the intended value) rotates which acknowledgment phrase is used so a
    field that gets retried more than once doesn't repeat the identical
    lead-in every time.
    """
    question = _ASK_QUESTIONS.get(metric, "Could you share more detail about your current recovery status?")
    ...
    if reason_code == DRC.FIELD_UNKNOWN_RETRY_REMAINING:
        ...
        ack = _pick(_AMBIGUOUS_ACK_POOL, variation_seed)
    elif reason_code == DRC.FIELD_PENDING:
        ack = _pick(_PENDING_ACK_POOL, variation_seed)
    else:
        ack = ""
...
_DECLINE_REASON_TEXT: Dict[str, str] = {
    recovery_logic.DecisionReasonCode.DAY_UNVERIFIED:
        "I'm not able to compare this to a day-specific target right now, since I can't verify your surgery date.",
    ...
    recovery_logic.DecisionReasonCode.EVIDENCE_MISSING:
        "I don't have supported evidence to give you a specific progress-milestone verdict right now.",
    recovery_logic.DecisionReasonCode.EVIDENCE_SOURCE_MISMATCH:
        "The evidence I found doesn't match the milestone I'd need to compare you against, so I can't give a supported verdict right now.",
    recovery_logic.DecisionReasonCode.EVIDENCE_PROCEDURE_MISMATCH:
        "The evidence I found is for a different procedure, so I can't give a supported verdict right now.",
    recovery_logic.DecisionReasonCode.MALFORMED_APPLICABILITY_WINDOW:
        "The guidance I found doesn't have a usable day range, so I can't give a supported verdict right now.",
    recovery_logic.DecisionReasonCode.DAY_OUTSIDE_APPLICABILITY_WINDOW:
        "The source I have only supports comparison within its stated day range, and your current day falls outside "
        "that range, so I can't give a supported verdict right now.",
    recovery_logic.DecisionReasonCode.RETRY_EXHAUSTED:
        "I wasn't able to get a clear answer on this after a couple of tries, so I can't give a supported verdict right now.",
    ...
    recovery_logic.DecisionReasonCode.INVALID_METRIC_VALUE:
        "That number doesn't look usable, so I can't rely on it for a supported verdict -- "
...
# Used only for CheckpointVerdict.TARGET_NOT_YET_DUE -- the checkpoint day
# hasn't been reached yet, so there is deliberately no range/target stated
# here (inventing one would be exactly the "Day 4 target" the patient never
# actually has). Deliberately NO trailing question: the state machine has
# no way to track or attribute an answer to a free-floating conversational
# question here (pending_field is already cleared by the time this text is
# built -- see the RecoverySessionState.set_fact() docstring), so asking one
# would be a question the agent could never actually follow up on. Every
# acknowledgment phrase below states a fact ("gives me a better picture" /
# "that helps" / "helps me understand where you are"), never a record-style
# confirmation like "noted".
...
_ASSESS_ACK_DUE_POOL: Tuple[str, ...] = (
...
def format_assess_message(checkpoint: "recovery_logic.CheckpointResult") -> str:
    """
    Checkpoint-relative ASSESS wording, phrased conversationally. Preserves
    the distinction between checkpoint_day (7, always TKA-03's own stated
    ...
    as though it were a target for the patient's current day, and no
    range/target is ever stated for a checkpoint that isn't due yet. Never
    uses "on track" / "above expected" / "ahead of schedule" / "normal
    recovery" or any other unsupported trajectory language.
    """
    metric_label = _metric_label(checkpoint.metric)
    value_text = f"{_fmt_num(checkpoint.patient_value)}°" if checkpoint.patient_value is not None else "that value"
    seed = int((checkpoint.effective_postop_day or 0) + (checkpoint.patient_value or 0))

    if checkpoint.verdict == recovery_logic.CheckpointVerdict.TARGET_NOT_YET_DUE:
        ack = _pick(_ASSESS_ACK_NOT_DUE_POOL, seed).format(value=value_text)
        body = (
            f"You're currently on post-op day {checkpoint.effective_postop_day}, and the first "
            f"{metric_label} checkpoint I have is for day {checkpoint.checkpoint_day}, so it's a "
            "little early to compare you against that target."
        )
        return f"{ack} {body}"
    ...
    ack = _pick(_ASSESS_ACK_DUE_POOL, seed)
    return f"{ack} your {metric_label} of {value_text} {relation} the {range_text} range stated for {day_clause}."
